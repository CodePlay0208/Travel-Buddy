...
import { editSecondaryKey, logout } from '../../../store/slices/auth-slice'
import { deleteProfile, generatePreSignedUrlForProfilePic, getProfile, updateProfile } from '../../../store/slices/profile-slice'
...
const UserDashboard = memo(({ userId }) => {
  ...
  const mapDataToApi = useCallback(async () => {
    ...
    const res = await dispatch(updateProfile(formData, true)).unwrap()
    if (res && imageFile) {
      const preSignedUrls = await dispatch(
  ...
  const handleSave = async (e) => {
    if (isUserKeyChanged) {
      await dispatch(
        editSecondaryKey(isLoginWithEmail ? { phoneNumber: `${formData.phoneNumber}` } : { emailId: formData.emailId }),
      ).unwrap()
  ...
  const handleDeleteAccount = async () => {
    ...
    const res = await dispatch(deleteProfile()).unwrap()
    if (res) {
